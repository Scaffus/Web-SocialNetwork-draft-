...
def new_post():
    ...
    if request.method == 'POST':
        ...
        if len(title) > 1 and len(content) > 10 and len(author_uuid) == 36:
            author = User.query.filter_by(uuid=author_uuid).first()
            author_name = author.username
            author.post_number += 1

            new_post = Post(author_uuid=author_uuid, author_name=author_name, title=title, content=content, uuid=str(uuid.uuid4()))
